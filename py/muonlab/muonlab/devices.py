...
class TempProbe(object):
    ...
    def set_mode16(self):
        self.d.i2c(self.address, [3, 0x80],
                   NumI2CBytesToReceive=0, **self.i2c_conf)
    ...
    def meas_temp(self):
        data = self.d.i2c(self.address, [],
                          NumI2CBytesToReceive=2, **self.i2c_conf)
        res = data['I2CBytes']
...
class UvaBias(object):
    ...
    def _convert_volt(self, ch, volt):
        ...
        if (volt <= self.lo_volt[ch - 1]):
            ...
            scale = self.hi_volt[ch - 1] - self.lo_volt[ch - 1]
            scale *= self.bit_range
            return int(scale * (self.hi_volt[ch - 1] - volt))
